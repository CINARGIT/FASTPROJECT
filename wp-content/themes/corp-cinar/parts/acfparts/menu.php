...
if( function_exists('acf_add_options_sub_page') ) {
	// Добавляем подстраницу к главной странице "Настройки темы"
...
function insert_iframe_into_acf_message() {
    ?>
    <script type="text/javascript">
    jQuery(document).ready(function($) {
        // Ищем наше поле "Сообщение" по уникальному идентификатору
        var messageField = $('#iframe_placeholder');
        
        if (messageField.length) {
            // Вставляем iframe
            var iframe = '<iframe src="/section_preview/10933/" width="100%" height="<?=get_field("menu_option", "option")["menu_sizes"]["height"]?>"></iframe>';
...
add_action('admin_footer', 'insert_iframe_into_acf_message');
...
acf_add_local_field_group(array(
    ...
    'fields' => array(
		...
		array(
                ...
                'name' => 'message_iframe',
                'type' => 'message',
                 'message' => '<div id="iframe_placeholder"></div>',
