		...
		<a href="<?php echo $link; ?>" <? if($args['openinmodal']) echo 'data-fancybox="review"';?>>
			<span class="sb_item_cart">
				...
				<? if(get_the_post_thumbnail_url( $args['content'], 'medium' )) { ?>
					<span class="sb_item_img"><img src="<?=get_the_post_thumbnail_url( $args['content'], 'medium' )?>" alt="<?php  echo $args['content']->post_title; ?>"></span>
				<? } ?>
				<span class="sb_item_cart_inf">
				<? if(get_sub_field('vyvodit_datu')) { ?>
				<span class="sb_item_date sb_item_atr"><?php echo get_the_date('d F Y', $args['content']->ID); ?></span>
				<? } ?>
