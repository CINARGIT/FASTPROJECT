...
if($showmode) {
	...
	<section class="section-common section-blog section-<?=$blogType?> <? if($sectionBg != '#ffffff') echo 'colored'; ?> <?=get_field('blog_select', 'styleset')?>" style="background-color:<?=$sectionBg?>">
	<div class="container">
		<h2><?=highlightLastWord(get_sub_field('zagolovok'))?></h2>
	...
	<? 
	 foreach ($myposts as $mypost) {
		get_template_part( 'parts/'.get_field('blog_select', 'styleset').'/item/'.$itemTemplate, null, $params = ['content' => $mypost, 'col' => $col, 'labelTextMore' => $labelTextMore, 'openinmodal' => $openInModal, 'dateoption' => $cOption['vyvodit_datu'], 'dop_gallery_img' => $cOption['dop_gallery_img']]);		
	 }
	?>	
	</div>	
	</div>	
	
	<? if(!empty(get_sub_field('blok_show_more'))) { ?>
