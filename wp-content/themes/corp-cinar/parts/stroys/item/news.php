	...
	<div class="col-xs-12 col-md-<?=$args['col']?> sb_item_col">
		...
		<a href="<?php echo $link; ?>" <? if($args['openinmodal']) echo 'data-fancybox="review"';?>>
			<span class="sb_item_cart">
				...
				<span class="row sb_item_row">
					<span class="sb_item_img sb_item_img_col col-xs-12 col-md-8">
						<img src="<?=get_the_post_thumbnail_url( $args['content'], 'medium' )?>" alt="<?php  echo $args['content']->post_title; ?>">
					</span>
					<span class="sb_item_dop_gallery_img sb_item_img_col  col-xs-12 col-md-4">
<?php 
$images = get_field('gallery', $args['content']); // замените 'gallery' на имя вашего поля галереи
if( $images ): $g = 0; ?>
    <span class="sb_item_gallery">
        <?php foreach( $images as $image ): $g++; ?>
            <div class="sb_item_gallery_item">
                  <img src="<?php echo esc_url($image['sizes']['thumbnail']); ?>" alt="<?php  echo $args['content']->post_title . ' ' . $g; ?>" />
            </div>
        <?php if($g==2) break; endforeach; ?>
    </span>
<?php endif; ?>
					</span>
				</span>
					...
					<span class="sb_item_sd">
						<? 
							if(!empty(get_field('kratoe_opisanie', $args['content']))) { 
								...
								echo custom_excerpt_length($args['content'], 17);
							}
						 ?>
					</span>
				</span>
