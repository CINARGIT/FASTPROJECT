    ...
    $abc = '<div class="cf7-dragandrophandler-container">
              <div class="cf7-dragandrophandler" data-type="'.$file_type_pattern.'" data-size="'.$allowed_size.'" data-max="'.$max.'">
                <div class="cf7-dragandrophandler-inner">
                    <div class="cf7-text-drop">'.__($text1,"contact-form-7-drop-files").'</div>
                    <div class="cf7-text-or">'.__($text2,"contact-form-7-drop-files").'</div>
                    <div class="cf7-text-browser"><a href="#">'.__($text3,"contact-form-7-drop-files").'</a></div>
                </div>
                <input type="file" class="input-uploads hidden" multiple>
            </div></div>';
    $html = sprintf(
        '<div class="wpcf7-form-control-wrap %1$s"><span class="wpcf7-form-control-wrap %1$s">'.$abc.'<input %2$s />%3$s</span></div>',
...
function wpcf7_drop_messages($messages){
    return array_merge( $messages, array(
        'invalid_file_min' => array(
            'description' => __( "Count files are smaller than the minimum allowed", "contact-form-7-drop-files" ),
            'default' => __( "Count files are smaller than the minimum allowed", "contact-form-7-drop-files" )
        ),

        'invalid_file_max' => array(
            'description' => __( "Count files are smaller than the maximum allowed", "contact-form-7-drop-files" ),
            'default' => __( "Count files are smaller than the maximum allowed", "contact-form-7-drop-files" )
        ),
    ) );
}
...
function wpcf7_add_tag_generator_file() {
    ...
    $tag_generator->add( 'dropfiles', __( 'Drop files', "contact-form-7-drop-files" ),
...
function wpcf7_tag_generator_file( $contact_form, $args = '' ) {
    ...
    <tbody>
        ...
        <tr>
        <th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-id' ); ?>"><?php echo esc_html( __( 'Min files', "contact-form-7-drop-files" ) ); ?></label></th>
        <td><input type="text" name="min" class="idvalue oneline option" id="<?php echo esc_attr( $args['content'] . '-min' ); ?>" /></td>
        </tr>
        
        <tr>
        <th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-id' ); ?>"><?php echo esc_html( __( 'Max files', "contact-form-7-drop-files" ) ); ?></label></th>
